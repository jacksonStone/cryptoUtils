...
module.exports = (keys) => { 
	if(!keys.hmackey || !keys.encryption) throw new Error('Must have hmackey and encryption keys in .env');
	ENCRYPTION_PASSWORD = keys.encryption.slice(0,16);
	HMAC_KEY = keys.hmackey.slice(0,16);
	return {
		getSalt,
		encrypt,
		decrypt,
